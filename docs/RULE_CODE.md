...
過去に `build.yml` が作った `build-<番号>` 形式のタグとReleaseが残っていると、
ストア側の更新判定が壊れる。`cleanup_tags.sh` を1度だけ実行して全削除する。
以後の採番は `v1.1.1` を起点に `v1.1.2`, `v1.1.3` と進む。

### 9-1-C. Release と stamp

`GITHUB_TOKEN` で作成した Release は**他のワークフローを起動しない**（GitHubの仕様）。
そのため `stamp.yml` の `on: release` は発火しない。
**sha256 の追記は `release.yml` の中で直接行う。** 本文の形式は次の1行。
...
**ディレクトリ単位の削除を行わない。** ファイルを1つ消すつもりで
`.github/workflows/` ごと消すと、カタログ管理システムがコミットした
`release.yml` や `stamp.yml` を巻き込む。削除は必ずファイル単位で行い、
実行後に `ls .github/workflows` で残存を確認する。

（実例: `build.yml` の削除時にディレクトリごと消し、`release.yml` を失って
Release が1件も作られなくなった。復旧まで数往復を要した）
